...
public class UserController {
     ...
     * 1.3 유저 피드 조회 API
     * [GET] /users/:userIdx
     * @return BaseReponse<GetUserFeedRes>
     */
    @ResponseBody
    @GetMapping("/{userIdx}")
    public BaseResponse<GetUserFeedRes> getUserFeedByIdx(@PathVariable("userIdx") int userIdx){
        // 현재 내 피드인지 다른 유저 피드인지 알기 위해선 현재 유저 인덱스와 지금 보려는 유저의 인덱스가 필요
        // 근데 아직 로그인을 안배웠으므로 (jwt) 임시로 userIdx를 두 번 넣어주자.
        try{
    ...
    /**
     * 1.4 유저 삭제 API -> 유저의 status를 ACTIVE -> DELETED로 수정
    ...
    public BaseResponse<DeleteUserRes> deleteUserByIdx(@PathVariable("userIdx") int userIdx){
        try{
            ...
            DeleteUserRes deleteUserRes = userService.deleteUserByIdx(deleteUserReq);
